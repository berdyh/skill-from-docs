...
**Security defaults.** `--allow-host HOST` required. Default redaction covers request headers (`Authorization`, `X-API-Key`, `X-Auth-Token`, `Api-Key`, `Token`, `Cookie`, `X-CSRF-Token`), URL query strings on credential-suggestive keys, response headers (same list plus `Set-Cookie`, `Location`), and JSON body keys on both the request and the response (`token`, `api_key`, `apiKey`, `secret`, `password`, `private_key`, `access_token`, `refresh_token`, `session`). Redacted content is what gets saved to disk and what `--dry-run` prints — real values never touch the filesystem under default policy. A request body that is not valid JSON is stored as text and only pattern-redaction (`--redact-body-pattern`) applies to it. `--no-redact` is opt-out and only documented for non-sensitive shared examples. Redirects are blocked by default; the `Location` header is captured but not followed automatically. On 429 with `Retry-After`, retry up to `--max-retries` times (default 3). On 5xx, exponential backoff (1s, 2s, 4s).
