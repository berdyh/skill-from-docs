...
**Strategy**: parse the spec programmatically — the rendered viewer is worthless to fetch. The discovery cascade lives in `openapi-harvest fetch` and is three steps: direct fetch → renderer-config regex over view-source (the five viewers above) → common spec paths against the origin. There is no fourth step; if all three fail, `fetch` exits 1 and finding a community mirror is your call, followed by a re-run of `fetch` against the mirror URL. Resolve `$ref`s with `prance[osv]` and emit a sidecar `source-map.json` so original JSON Pointers remain accurate after the flatten. Group endpoints by OpenAPI tag under per-tag H3 sub-sections beneath `## API reference` (per `doc-template.md`). Merge prose context — what the tool is, auth flow, rate limits, error semantics — from sibling narrative pages or the landing page. Every section carries a JSON Pointer provenance comment (`spec-pointer: /paths/~1v1~1locations/get`); `consolidate` additionally stamps `mirror: unofficial` on every spec-source comment when the spec's source host differs from the host its own `servers` block declares. That is a factual observation, not a judgement about the maintainer: a vendor's own spec published to a code-hosting site looks identical from here to a third-party re-host, so read the label as "verify this source", not "distrust it". The label is absent when either host is unknown, so absence never means "verified first-party".
